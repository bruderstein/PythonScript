...
class ReplaceAnsiPythonFunctionTestCase(unittest.TestCase):
    ...
    def test_utf8_replace_function(self):
        editor.replace2(ur'([a-zäöü]+)([0-9]+)'.encode('windows-1252'), group1_with_counter)
        text = editor.getText()
    ...
    def test_groups(self):
        editor.replace2(ur'([a-zäöü]+)([0-9]+)'.encode('windows-1252'), lambda m: self.groups_check(m))
        text = editor.getText()
        self.assertEqual(text, '1 2 3\r\n4 5 6\r\n')


    def test_groups_with_named_groups(self):
        editor.replace2(ur'(?<letters>[a-zäöü]+)(?<numbers>[0-9]+)'.encode('windows-1252'), lambda m: self.groups_check(m))
        text = editor.getText()
        self.assertEqual(text, '1 2 3\r\n4 5 6\r\n')
    ...
    def test_named_groups_2(self):
        editor.replace2(ur'(?<letters>[a-zäöü]+)(?<numbers>[0-9]+)'.encode('windows-1252'), lambda m: m.group('numbers'))
        text = editor.getText()
    ...
    def group_tuples_check(self, m):
        ...
        groups_data_correct = { 1 : ('123', 'abc', '123'), 
                            2 : ('5432', 'def', '5432'),
                            3 : ('98', 'gh', '98'),
                            4 : ('123', u'äbc'.encode('windows-1252'), '123'),
                            5 : ('5432', 'üef'.encode('windows-1252'), '5432'),
                            6 : ('98', 'öh'.encode('windows-1252'), '98') }
        self.assertEqual(m.group(2, 'letters', 'numbers'), groups_data_correct[counter])
        return counter
        
    def test_group_tuples(self):
        editor.replace2(ur'(?<letters>[a-zäöü]+)(?<numbers>[0-9]+)'.encode('windows-1252'), lambda m: self.group_tuples_check(m))
        text = editor.getText()
        self.assertEqual(text, '1 2 3\r\n4 5 6\r\n')
