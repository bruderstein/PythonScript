...
BOOST_PYTHON_MODULE(Npp)
{
	...
	class_<ScintillaWrapper>("Editor", no_init)
		...
		.def("replace", &ScintillaWrapper::replace, "Simple search and replace. replace(searchFor, replaceWith)")
		.def("rereplace", &ScintillaWrapper::rereplace, "Simple regular expression search and replace (using Notepad++/Scintilla regular expressions).  rereplace(searchExpression, replaceString)")
		.def("pyreplace", &ScintillaWrapper::pyreplace, "Python regular expression search and replace. Full support for Python regular expressions.  Works line-by-line, so does not require significant memory overhead, however multiline regular expressions won't work (see pymlreplace).  editor.pyreplace(search, replace[, count]).  Uses the python re.sub() method.")
		.def("pyreplace", &ScintillaWrapper::pyreplaceNoCount, "Python regular expression search and replace. Full support for Python regular expressions.  Works line-by-line, so does not require significant memory overhead, however multiline regular expressions won't work (see pymlreplace).  editor.pyreplace(search, replace[, count]).  Uses the python re.sub() method.")
		.def("pymlreplace", &ScintillaWrapper::pymlreplace, "Python Multiline regular expression search and replace - works for multiline regular expressions, but makes at least 2 copies of the entire document, so is unsuitable for large documents. editor.pymlreplace(search, replace[, count]).  Uses the python re.sub() method.")
		.def("pymlreplace", &ScintillaWrapper::pymlreplaceNoCount, "Python Multiline regular expression search and replace - works for multiline regular expressions, but makes at least 2 copies of the entire document, so is unsuitable for large documents. editor.pymlreplace(search, replace).  Uses the python re.sub() method.")
